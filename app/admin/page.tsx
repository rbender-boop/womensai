...
const CAT_COLORS: Record<string, string> = {
  health: '#9B4163', fitness: '#3B82F6', wellness: '#10B981', beauty: '#EC4899',
  nutrition: '#F59E0B', mental_health: '#8B5CF6', other: '#6B7280',
};

const CHANNEL_LABELS: Record<string, string> = {
  email_self: 'Email to Self', email_friend: 'Email to Friend', link: 'Copy Link',
  x: 'X / Twitter', facebook: 'Facebook', instagram: 'Instagram', native_share: 'Native Share',
};
...
function Sparkline({ data, color = '#9B4163' }: { data: number[]; color?: string }) {
  ...
  return (
    <div className="flex items-end gap-[2px] h-8">
      {data.map((v, i) => (
        <div key={i} className="rounded-sm min-w-[3px] flex-1 transition-all"
          style={{ height: `${Math.max((v / max) * 100, 4)}%`, background: v > 0 ? color : 'rgba(0,0,0,0.06)', opacity: i === data.length - 1 ? 1 : 0.7 }} />
      ))}
    </div>
  );
}
...
function LoginScreen({ onAuth }: { onAuth: () => void }) {
  ...
  async function handleLogin() {
    ...
    try {
      ...
      if (res.ok) onAuth(); else setErr('Invalid password');
  ...
  return (
    <div className="min-h-screen flex items-center justify-center" style={{ background: 'linear-gradient(160deg, #FDF5F8 0%, #FBF8F5 45%, #F6EFF9 100%)' }}>
      <div className="w-full max-w-sm p-8 rounded-3xl" style={{ background: 'rgba(255,255,255,0.85)', border: '1.5px solid rgba(212,167,185,0.4)', backdropFilter: 'blur(16px)', boxShadow: '0 8px 40px rgba(139,48,88,0.08)' }}>
        ...
        <input type="password" placeholder="Password" value={pw} onChange={(e) => { setPw(e.target.value); setErr(''); }}
          ...
          style={{ background: 'rgba(253,245,248,0.8)', border: '1px solid rgba(212,167,185,0.4)', color: '#1C1714' }} autoFocus />
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    fetch('/api/admin/stats?range=7').then((r) => { if (r.ok) setAuthed(true); }).catch(() => {}).finally(() => setCheckingAuth(false));
  }, []);

  const loadStats = useCallback(async () => {
    setLoading(true);
    try { const res = await fetch(`/api/admin/stats?range=${range}`); if (res.ok) setStats(await res.json()); } catch {}
    setLoading(false);
  }, [range]);
  ...
  useEffect(() => { if (authed && tab === 'users') loadUsers(); }, [authed, tab, loadUsers]);
        ...
        <div className="flex gap-1 p-1 rounded-2xl" style={{ background: 'rgba(255,255,255,0.5)', border: '1px solid rgba(212,167,185,0.2)' }}>
          {tabs.map(({ id, label, icon: Icon }) => (
            <button key={id} onClick={() => setTab(id)}
              ...
              style={tab === id ? { background: 'rgba(255,255,255,0.9)', color: '#9B4163', boxShadow: '0 2px 8px rgba(139,48,88,0.08)', border: '1px solid rgba(212,167,185,0.3)' }
                : { color: '#7A6E67', background: 'transparent', border: '1px solid transparent' }}>
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              {[
                ...
                <div key={label} className="p-5 rounded-2xl" style={{ background: 'rgba(255,255,255,0.75)', border: '1px solid rgba(212,167,185,0.25)', backdropFilter: 'blur(8px)' }}>
                  <div className="flex items-center gap-2 mb-3"><Icon size={14} style={{ color }} /><span className="text-xs font-medium" style={{ color: '#7A6E67' }}>{label}</span></div>
                  <div className="flex items-end justify-between gap-4">
                    ...
                    <div className="w-20"><Sparkline data={spark} color={color} /></div>
                  </div>
                </div>
              ))}
            </div>
              ...
              <div className="p-5 rounded-2xl" style={{ background: 'rgba(255,255,255,0.75)', border: '1px solid rgba(212,167,185,0.25)' }}>
                ...
                <div className="space-y-2.5">
                  {Object.entries(stats?.categoryBreakdown || {}).sort((a, b) => b[1] - a[1]).map(([cat, count]) => (
                    <HBar key={cat} label={cat.replace('_', ' ')} value={count} max={Math.max(...Object.values(stats?.categoryBreakdown || {}))} color={CAT_COLORS[cat] || '#6B7280'} />
                  ))}
                  {Object.keys(stats?.categoryBreakdown || {}).length === 0 && <p className="text-xs" style={{ color: '#AFA8A2' }}>No category data yet</p>}
                </div>
              </div>
              <div className="p-5 rounded-2xl" style={{ background: 'rgba(255,255,255,0.75)', border: '1px solid rgba(212,167,185,0.25)' }}>
                ...
                <div className="space-y-2">
                  {(stats?.topTopics || []).slice(0, 10).map(({ topic, count }, i) => (
                    <div key={topic} className="flex items-center gap-3">
                      ...
                      <span className="text-xs px-2 py-0.5 rounded-full" style={{ background: 'rgba(155,65,99,0.06)', color: '#9B4163', border: '1px solid rgba(155,65,99,0.12)' }}>{topic.replace(/_/g, ' ')}</span>
                      <span className="ml-auto text-xs font-semibold" style={{ color: '#1C1714' }}>{count}</span>
                    </div>
                  ))}
                  {(stats?.topTopics || []).length === 0 && <p className="text-xs" style={{ color: '#AFA8A2' }}>No topics tagged yet</p>}
                </div>
              </div>
            </div>
          </div>
                  ...
                  {(stats?.recentQuestions || []).map((q) => (
                    <tr key={q.id} className="border-t" style={{ borderColor: 'rgba(212,167,185,0.1)' }}>
                      ...
                      <td className="px-4 py-2.5">
                        {q.category && <span className="px-2 py-0.5 rounded-full" style={{ background: `${CAT_COLORS[q.category] || '#6B7280'}15`, color: CAT_COLORS[q.category] || '#6B7280', fontSize: '10px' }}>{q.category.replace('_', ' ')}</span>}
                      </td>
                      ...
                      <td className="px-4 py-2.5">
                        <span className="px-2 py-0.5 rounded-full" style={{ fontSize: '10px', background: q.status === 'success' ? 'rgba(16,185,129,0.1)' : 'rgba(239,68,68,0.1)', color: q.status === 'success' ? '#10B981' : '#EF4444' }}>{q.status}</span>
                      </td>
                    </tr>
                  ))}
                </tbody>
              </table>
              {(stats?.recentQuestions || []).length === 0 && <p className="text-center text-xs py-8" style={{ color: '#AFA8A2' }}>No questions yet</p>}
            </div>
          </div>
        )}
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <div className="p-5 rounded-2xl" style={{ background: 'rgba(255,255,255,0.75)', border: '1px solid rgba(212,167,185,0.25)' }}>
                ...
                <div className="space-y-2.5">
                  {Object.entries(stats?.sharesByChannel || {}).sort((a, b) => b[1] - a[1]).map(([ch, count]) => (
                    <HBar key={ch} label={CHANNEL_LABELS[ch] || ch} value={count} max={Math.max(...Object.values(stats?.sharesByChannel || {}), 1)} color="#10B981" />
                  ))}
                  {Object.keys(stats?.sharesByChannel || {}).length === 0 && <p className="text-xs" style={{ color: '#AFA8A2' }}>No shares yet</p>}
                </div>
              </div>
              <div className="p-5 rounded-2xl" style={{ background: 'rgba(255,255,255,0.75)', border: '1px solid rgba(212,167,185,0.25)' }}>
                <h3 className="text-sm font-semibold mb-4" style={{ color: '#1C1714' }}>Daily Shares</h3>
                <div className="h-40"><Sparkline data={Object.values(stats?.daily?.shares || {})} color="#10B981" /></div>
            ...
            {/* Email Share Log */}
            <div className="rounded-2xl overflow-hidden" style={{ background: 'rgba(255,255,255,0.75)', border: '1px solid rgba(212,167,185,0.25)' }}>
              <div className="p-5" style={{ borderBottom: '1px solid rgba(212,167,185,0.15)' }}>
                <h3 className="text-sm font-semibold" style={{ color: '#1C1714' }}>Email Share Log</h3>
                <p className="text-xs mt-0.5" style={{ color: '#AFA8A2' }}>Who shared, to whom, and what question</p>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full text-xs">
                  <thead>
                    <tr style={{ background: 'rgba(253,245,248,0.5)' }}>
                      <th className="text-left px-4 py-2.5 font-medium" style={{ color: '#7A6E67' }}>Time</th>
                      <th className="text-left px-4 py-2.5 font-medium" style={{ color: '#7A6E67' }}>Channel</th>
                      <th className="text-left px-4 py-2.5 font-medium" style={{ color: '#7A6E67' }}>Sender</th>
                      <th className="text-left px-4 py-2.5 font-medium" style={{ color: '#7A6E67' }}>Recipient</th>
                      <th className="text-left px-4 py-2.5 font-medium" style={{ color: '#7A6E67' }}>Question</th>
                    </tr>
                  </thead>
                  <tbody>
                    {(stats?.recentEmailShares || []).map((s) => (
                      <tr key={s.id} className="border-t" style={{ borderColor: 'rgba(212,167,185,0.1)' }}>
                        <td className="px-4 py-2.5 whitespace-nowrap" style={{ color: '#AFA8A2' }}>
                          {new Date(s.created_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}{' '}
                          {new Date(s.created_at).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}
                        </td>
                        <td className="px-4 py-2.5">
                          <span className="px-2 py-0.5 rounded-full" style={{ fontSize: '10px', background: s.share_channel === 'email_friend' ? 'rgba(155,65,99,0.08)' : 'rgba(59,130,246,0.08)', color: s.share_channel === 'email_friend' ? '#9B4163' : '#3B82F6' }}>
                            {CHANNEL_LABELS[s.share_channel] || s.share_channel}
                          </span>
                        </td>
                        <td className="px-4 py-2.5 font-medium" style={{ color: '#1C1714' }}>{s.sender_email || '\u2014'}</td>
                        <td className="px-4 py-2.5 font-medium" style={{ color: '#9B4163' }}>{s.recipient_email || '\u2014'}</td>
                        <td className="px-4 py-2.5 max-w-[250px] truncate" style={{ color: '#7A6E67' }}>{s.query_text || '\u2014'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                {(stats?.recentEmailShares || []).length === 0 && <p className="text-center text-xs py-8" style={{ color: '#AFA8A2' }}>No email shares yet</p>}
              </div>
            </div>
          </div>
        )}
        ...
        {tab === 'users' && (
          <div className="rounded-2xl overflow-hidden" style={{ background: 'rgba(255,255,255,0.75)', border: '1px solid rgba(212,167,185,0.25)' }}>
            <div className="p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3" style={{ borderBottom: '1px solid rgba(212,167,185,0.15)' }}>
              ...
              <div className="flex items-center gap-2">
                <div className="relative">
                  ...
                  <input type="text" placeholder="Search email..." value={userSearch}
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-xs">
                <thead>
                  <tr style={{ background: 'rgba(253,245,248,0.5)' }}>
                    ...
                    <th className="text-left px-4 py-2.5 font-medium" style={{ color: '#7A6E67' }}>Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {(users?.users || []).map((u) => (
                    <tr key={u.id} className="border-t" style={{ borderColor: 'rgba(212,167,185,0.1)' }}>
                      ...
                      <td className="px-4 py-2.5">
                        <div className="flex gap-1 flex-wrap">
                          {Object.entries(u.questionCategories).sort((a, b) => b[1] - a[1]).slice(0, 3).map(([cat, cnt]) => (
                            <span key={cat} className="px-1.5 py-0.5 rounded" style={{ fontSize: '9px', background: `${CAT_COLORS[cat] || '#6B7280'}12`, color: CAT_COLORS[cat] || '#6B7280' }}>
                              {cat.replace('_', ' ')} ({cnt})
                            </span>
                          ))}
                          {Object.keys(u.questionCategories).length === 0 && <span style={{ color: '#AFA8A2' }}>{'\u2014'}</span>}
                        </div>
                      </td>
                      <td className="px-4 py-2.5">
                        {deleteConfirm === u.email ? (
                          <div className="flex items-center gap-1">
                            <button onClick={() => deleteUser(u.email)}
                              className="text-[10px] px-2 py-1 rounded font-semibold" style={{ background: '#EF4444', color: '#fff' }}>Confirm</button>
                            <button onClick={() => setDeleteConfirm(null)}
                              className="text-[10px] px-2 py-1 rounded" style={{ background: '#EDE8E3', color: '#7A6E67' }}>Cancel</button>
                          </div>
                        ) : (
                          <button onClick={() => setDeleteConfirm(u.email)} className="p-1.5 rounded-lg transition-colors hover:bg-red-50" title="Delete user">
              ...
              {(users?.users || []).length === 0 && <p className="text-center text-xs py-8" style={{ color: '#AFA8A2' }}>No users found</p>}
            </div>
            {users && users.totalPages > 1 && (
              <div className="flex items-center justify-between px-5 py-3" style={{ borderTop: '1px solid rgba(212,167,185,0.15)' }}>
                <button onClick={() => setUserPage(Math.max(1, userPage - 1))} disabled={userPage === 1}
                  className="flex items-center gap-1 text-xs px-3 py-1.5 rounded-lg disabled:opacity-30" style={{ color: '#7A6E67', background: 'rgba(253,245,248,0.5)' }}>
                  <ChevronLeft size={12} /> Prev
                </button>
                <span className="text-xs" style={{ color: '#AFA8A2' }}>Page {userPage} of {users.totalPages}</span>
                <button onClick={() => setUserPage(Math.min(users.totalPages, userPage + 1))} disabled={userPage === users.totalPages}
                  className="flex items-center gap-1 text-xs px-3 py-1.5 rounded-lg disabled:opacity-30" style={{ color: '#7A6E67', background: 'rgba(253,245,248,0.5)' }}>
            ...
            <div className="p-5 rounded-2xl" style={{ background: 'rgba(255,255,255,0.75)', border: '1px solid rgba(212,167,185,0.25)' }}>
              ...
              <div className="space-y-2.5">
                {(stats?.topPaths || []).map(({ path, count }) => (
                  <HBar key={path} label={path === '/' ? 'Homepage' : path.length > 18 ? path.slice(0, 18) + '...' : path}
                    value={count} max={Math.max(...(stats?.topPaths || []).map((p) => p.count), 1)} color="#3B82F6" />
                ))}
                {(stats?.topPaths || []).length === 0 && <p className="text-xs" style={{ color: '#AFA8A2' }}>No page view data yet</p>}
              </div>
            </div>
            <div className="p-5 rounded-2xl" style={{ background: 'rgba(255,255,255,0.75)', border: '1px solid rgba(212,167,185,0.25)' }}>
              <h3 className="text-sm font-semibold mb-4" style={{ color: '#1C1714' }}>Daily Page Views</h3>
              <div className="h-40"><Sparkline data={Object.values(stats?.daily?.views || {})} color="#3B82F6" /></div>
              <div className="flex justify-between mt-2">
